...
export function calcAttendancePct(employeeId, attendance) {
  const rec = attendance.find(a => String(a.employeeId) === String(employeeId));
...
export function isOnLeaveToday(employeeId, leaves) {
  ...
  return leaves.some(l => {
    if (String(l.employeeId) !== String(employeeId) || l.status !== 'Approved') return false;
...
export function detectLeaveConflict(newLeave, existingLeaves) {
  return existingLeaves
    .filter(l => String(l.employeeId) === String(newLeave.employeeId) && l.status !== 'Rejected' && l.id !== newLeave.id)
    .some(l => {
...
export function calcLeaveBalance(employeeId, leaves) {
  ...
  const used = leaves
    .filter(l => String(l.employeeId) === String(employeeId) && l.status === 'Approved')
    .reduce((sum, l) => {
